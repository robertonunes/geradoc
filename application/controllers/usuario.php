...
class Usuario extends CI_Controller {
	...
	public function update($id, $disabled = null) {
		...
		if ($this->form_validation->run($this->area."/edit") == FALSE) {
			...
			//echo print_r($this->input->post('campoSetores'));
	...
	function email($cpf, $email, $nova_senha){
		...
		$mensagem .= '</div>';
		$mensagem .= "GeraDoc - Sistema Gerenciador de Documentos da AESP/CE<br />";
		$mensagem .= "CTIC - Célula de Tecnologia da Informação e Comunicação<br />";
		$mensagem .= "(85) 3296-0015";
	
		$list = array($email);
		$this->load->library('email');
		$this->email->from('ctic@aesp.ce.gov.br', 'CTIC-AESP');
	
		$this->email->reply_to('ctic@aesp.ce.gov.br');
		$this->email->to($list);
		//$this->email->bcc('tarsodecastro@gmail.com');
 ...
 public function search($page = 1) { 
    ...
    function view($id){
    	...
    	if($data['objeto']->setores != null){
    			
    		$setores_usuario = $data['objeto']->setores;
    		...
    		if(strpos($setores_usuario, ';') > 0){
    			...
    			$nome_setor = $this->_get_setor($data['objeto']->setores);
    				
    		}
    			
    	}else{
    			
    		$nome_setor = $this->_get_setor($data['objeto']->setor);
    	}
    	
    	$data['objeto']->setor = $nome_setor;
    	
    	switch ($data['objeto']->nivel) {
    		case "1":
    			$data['objeto']->nivel = "ADMINISTRADOR";
    			break;
    		case "2":
    			$data['objeto']->nivel = "REDATOR";
    			break;
    	}
    ...
    function _get_setor($id_setor){
    	...
    	//$obj = $this->Usuario_model->get_setor($id_usuario)->row();
    	...
    	//$setor = str_replace('/NENHUM', '', $obj->sigla);
